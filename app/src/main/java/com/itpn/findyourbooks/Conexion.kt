...
class Conexion(context: Context?,
        ...
        SQLiteOpenHelper(context, name, factory, version) {
            ...
            override fun onCreate(p0: SQLiteDatabase?) {
                ...
                p0?.execSQL("CREATE TABLE IF NOT EXISTS libro (idlibro INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, titulo TEXT NOT NULL, tema_idtema INTEGER NOT NULL, autor_idautor INTEGER NOT NULL, tienda_idtienda INTEGER NOT NULL , CONSTRAINT fk_libro_tema FOREIGN KEY (tema_idtema) REFERENCES tema (idtema) ON DELETE CASCADE ON UPDATE CASCADE , CONSTRAINT fk_libro_autor1 FOREIGN KEY (autor_idautor) REFERENCES autor (idautor) ON DELETE CASCADE ON UPDATE CASCADE , CONSTRAINT fk_tienda_libro FOREIGN KEY (tienda_idtienda) REFERENCES tienda (idtienda) ON DELETE CASCADE ON UPDATE CASCADE);")
